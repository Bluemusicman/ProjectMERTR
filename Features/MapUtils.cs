...
public static class MapUtils
{
	...
	public static MapSchematic GetMapData(string mapName)
	{
		...
		if (!File.Exists(path))
		{
			string error = $"Harita verisi yüklenemedi: {mapName}.yml dosyası mevcut değil!";
		...
		{
			string error = $"Harita verisi yüklenemedi: {mapName}.yml dosyasında YAML hataları var!\n{e.ToString().Split('\n')[0]}";
	...
	public static SchematicObjectDataList GetSchematicDataByName(string schematicName)
	{
		...
		if (!Directory.Exists(schematicDirPath))
		{
			...
			string error = $"Şematik verisi yüklenemedi: {schematicName} klasörü mevcut değil!";
			Logger.Error(error);
		...
		if (!File.Exists(schematicJsonPath))
		{
			...
			string error = $"Şematik verisi yüklenemedi: {schematicName}.json dosyası mevcut değil!";
			Logger.Error(error);
			throw new FileNotFoundException(error);
		}
		...
		{
			string error = $"Şematik verisi yüklenemedi: {schematicName}.json dosyasında JSON hataları var!\n{e.ToString().Split('\n')[0]}";
			Logger.Error(error);
